...
- (void)handleShare:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  } else if ([@"shareEmoticon" isEqualToString:call.method]) {
    [self shareEmoticon:call result:result];
  }
}
...
- (void)shareImage:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  dispatch_async(globalQueue, ^{
    ...
    if (thumbnailImage == nil) {
      ...
      realThumbnailImage = [self getThumbnailFromNSData:sourceImageData
...
- (void)shareFile:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  dispatch_async(globalQueue, ^{
    ...
    if ([suffix hasPrefix:@"."]) {
      ...
      fileExtension = [suffix stringByReplacingCharactersInRange:range
                                                      withString:@""];
    }
...
- (void)shareMiniProgram:(FlutterMethodCall *)call
  ...
  dispatch_async(globalQueue, ^{
    ...
    if (hdImagePath != (id)[NSNull null]) {
      ...
      hdImageData = [self getThumbnailDataFromNSData:imageData
                                                size:120 * 1024
                                            compress:compress];
      //            UIImage *uiImage = [self getThumbnailFromNSData:imageData
      //            size:120 * 1024 isPNG:isPNG compress:compress]; if (isPNG) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      [WXApiRequestHandler
          sendMiniProgramWebpageUrl:call.arguments[@"webPageUrl"]
                    ...
                    withShareTicket:[call.arguments[@"withShareTicket"]
                                        boolValue]
...
- (void)shareEmoticon:(FlutterMethodCall *)call result:(FlutterResult)result {
  dispatch_queue_t globalQueue = dispatch_get_global_queue(0, 0);
  dispatch_async(globalQueue, ^{
    NSDictionary *sourceImage = call.arguments[keyEmoticon];
    NSData *emoticonData = [self getNsDataFromWeChatFile:sourceImage];
    ...
    if (thumbnailImage == nil) {
      ...
      realThumbnailImage = [self getThumbnailFromNSData:emoticonData
                                                   size:defaultThumbnailSize
                                                  isPNG:isPNG
                                               compress:compress];
    } else {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      NSNumber *scene = call.arguments[fluwxKeyScene];
        [WXApiRequestHandler sendEmotionData:emoticonData
                                  ThumbImage:realThumbnailImage
                                     InScene:[self intToWeChatScene:scene]
                                MsgSignature:call.arguments[fluwxKeyMsgSignature] completion:^(BOOL success) {
            result(@(success));
        }];
    });
  });
...
- (UIImage *)getCommonThumbnail:(FlutterMethodCall *)call {
  ...
  UIImage *thumbnailImage = [self getThumbnailFromNSData:thumbnailData
                                                    size:defaultThumbnailSize
                                                   isPNG:[self isPNG:suffix]
                                                compress:[compress boolValue]];
...
- (UIImage *)getThumbnailFromNSData:(NSData *)data
...
- (NSData *)getThumbnailDataFromNSData:(NSData *)data
                                  size:(NSUInteger)size
                              compress:(BOOL)compress {
  if (compress) {
...
- (NSArray *)formatAssets:(NSString *)originPath {
  ...
  if (indexOfPackage != JavaNotFound) {
    ...
    packageName =
        [pathWithoutSchema substringFromIndex:begin
                                      toIndex:[pathWithoutSchema length]];
  } else {
